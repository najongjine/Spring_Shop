 ...
 * 정상 로그인된 사용자 정보가 token 형태로 default-target-url에 지정된
...
public class CustomAuthProvider implements AuthenticationProvider {
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		/*
		 * 만약 해당 사용자 Detail 정보가 DB에 있다면 해당 정보를 DB에서 조회하여 CustomUserDetails 클래스에
		 * 셋팅하여 controller로 전송할수 있다.
		 */
		CustomUserDetails cUserDetails=new CustomUserDetails();
		cUserDetails.setNick_name("홍길동");
		cUserDetails.setTel("010-000-0000");
