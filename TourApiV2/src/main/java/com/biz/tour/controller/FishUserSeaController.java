...
public class FishUserSeaController {
	...
	public String insert(Model model) {
		// 로그인한 사용자 정보
		UserDetailsVO secUserVO = (UserDetailsVO) SecurityContextHolder.getContext().getAuthentication().getPrincipal();

		// 권한(ROLE) 리스트 추출하여 userVO에 setting
		secUserVO.setAuthorities(SecurityContextHolder.getContext().getAuthentication().getAuthorities());
		FishUserSeaVO userVO = new FishUserSeaVO();
		userVO.setUf_username(secUserVO.getUsername());
	...
	public String insert(FishUserSeaVO userVO, MultipartHttpServletRequest uploaded_files, HttpSession session,
			Model model) {
		// 로그인한 사용자 정보
		UserDetailsVO secUserVO = (UserDetailsVO) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
	...
	public String update(Model model, String strId) {
		...
		if (!userVO.getUf_username().equals(secUserVO.getUsername())) {
			return null;
		}
	...
	public String update(FishUserSeaVO userVO, MultipartHttpServletRequest uploaded_files, Model model, String strId) {
		Long uf_id = (long) -1;
		try {
			uf_id = Long.valueOf(strId);
		} catch (Exception e) {
	...
	public String comments(@RequestParam("ufc_fk") String strFk,
			@RequestParam(value = "pageno", defaultValue = "1") String strPageno, Model model) {
		int pageno = Integer.valueOf(strPageno);
		long ufc_fk = -1;
		int maxListSize = -1;
	...
	public String comments(FishUserSeaCommentVO commentVO,
			@RequestParam(value = "pageno", defaultValue = "1") String strPageno, HttpSession session, Model model) {
		int pageno = Integer.valueOf(strPageno);
		int maxListSize = -1;
		int ret = uSCommentService.insert(commentVO, session);
		maxListSize = uSCommentService.countFindByFk(commentVO.getUfc_fk());
		List<FishUserSeaCommentVO> commentList = uSCommentService.findByFk(commentVO.getUfc_fk(), (pageno - 1),
				itemLimit);
		model.addAttribute("commentList", commentList);
